...
def interactive_setup_node(state: NovelProblemState) -> dict:
    ...
    if "Random Topic" in strategy:
        ...
        if not questionary.confirm("Shall I proceed with these topics?", default=True).ask():
        ...
        clean_prompt = "\n".join([line for line in user_prompt.splitlines() if not line.strip().startswith('#')]).strip()
        if not clean_prompt:
            print("No idea provided. Aborting.")
            raise KeyboardInterrupt
        user_prompt = clean_prompt

    return {"strategy": strategy, "topics": topics, "user_prompt": user_prompt, "previous_problem": previous_problem}
...
def interactive_feedback_node(state: NovelProblemState) -> dict:
    ...
    with tempfile.NamedTemporaryFile(mode='w+', delete=False, suffix=".txt", encoding="utf-8") as tf:
        feedback_file_path = Path(tf.name)
        tf.write("# Please enter your feedback below. Save and close this file to continue.\n# To accept the current version, save and close this file without adding any other text.\n")
    open_in_editor(feedback_file_path)
