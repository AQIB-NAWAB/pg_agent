...
from pg_agent.utils.env import load_env, get_available_models, default_model, get_settings
...
from pg_agent.utils.parsing import extract_cpp_code, extract_python_code, extract_code
...
async def generate_one(task_id, llm, prompt, index, problem_paths, model_name, logger, progress_tracker, language="C++"):
    messages = [HumanMessage(content=prompt)]
    file_ext = "cpp" if language == "C++" else "py"
    language_name = language.lower().replace("+", "p")
    
    # Try up to 3 attempts to get valid code
    for attempt in range(3):
        try:
            # Get paths for code, prompt and raw response
            code_path, prompt_path, response_path, reasoning_path = problem_paths.get_run_paths(model_name, index, language)
            ...
            code = extract_code(response.content, language)
            
            # If code is empty, try extracting from reasoning file
            if not code and reasoning_path.exists():
                logger.debug(f"Attempt {attempt + 1}: No {language} code found in response, trying reasoning file")
                try:
                    reasoning_content = reasoning_path.read_text(encoding="utf-8")
                    if reasoning_content.strip():
                        code = extract_code(reasoning_content, language)
                except Exception as read_error:
                    logger.debug(f"Could not read reasoning file: {read_error}")
            
            # If we still don't have code and this isn't the last attempt, retry
            if not code and attempt < 2:
                logger.warning(f"Attempt {attempt + 1}: No {language} code found, retrying... ({attempt + 2}/3)")
                continue
            
            # If we still don't have code after all attempts, log error but save what we have
            if not code:
                logger.error(f"❌ No {language} code found after 3 attempts for solution {index}")
                comment_prefix = "//" if language == "C++" else "#"
                code = f"{comment_prefix} No {language} code could be extracted from the response"
            ...
            # If we got here with valid code, break out of retry loop
            comment_prefix = "//" if language == "C++" else "#"
            if code and code != f"{comment_prefix} No {language} code could be extracted from the response":
                break
...
async def generate_code_async(llm_configs, problem_text, num, problem_paths, logger, language="C++"):
    """
    ...
    if language == "Python":
        prompt = (
            ...
            f"{problem_text}\n\n"
            "First analyze the problem, then provide your solution in Python. "
            ...
            "Your final solution should be a complete, runnable Python program."
        )
    else:
        prompt = (
            "You are an expert competitive programmer. Please solve the following problem:\n\n"
    ...
    for llm, model_name in llm_configs:
        ...
        max_index = 0
        if model_dir.exists():
            for file in model_dir.glob(f"run_*.{file_ext}"):
                match = re.search(rf"run_(\d+)\.{file_ext}", file.name)
                if match:
        ...
        _, prompt_path, _, _ = problem_paths.get_run_paths(model_name, 0, language)
    ...
    print(f"  Code     → {relative_problem_dir}/runs/<model_name>/run_<num>.{file_ext}")
    ...
    for model_info_item in model_info:
        ...
        run_numbers = ", ".join(f"run_{i:02d}.{file_ext}" for i in indexes)
    ...
    for model_info_item in model_info:
        ...
        for i in indexes:
            tasks.append(
                generate_one(task_id, llm, prompt, i, problem_paths, model_name, logger, progress_tracker, language)
            )
            task_id += 1
...
def main():
    ...
    try:
        ...
        asyncio.run(generate_code_async(llm_configs, problem_text, args.num, problem_paths, logger, language))
